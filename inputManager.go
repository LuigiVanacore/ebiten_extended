...
var inputManagerInstance *inputManager

func InputManager() *inputManager {
	if inputManagerInstance == nil {
		inputManagerInstance = newInputManager()
	}
...
type inputManager struct {
	actionMap 	 *inputv3.ActionMap
	inputBuffer	   *inputv3.InputBuffer
	mouseEnabled       bool
	mouseStartPosition math2D.Vector2D
	mouseDragPosition  math2D.Vector2D
	cursorPosition     math2D.Vector2D

}

func newInputManager() *inputManager {
	inputManager := &inputManager{
		actionMap:  inputv3.NewActionMap(),
		inputBuffer: inputv3.NewInputBuffer(),
	}
	inputManager.init()
	return inputManager
}
...
func (i *inputManager) init() {
	...
	i.cursorPosition = math2D.NewVector2D(0, 0)
	i.mouseStartPosition = math2D.NewVector2D(0, 0)
	i.mouseDragPosition = math2D.NewVector2D(0, 0)
}
...
func (i *inputManager) SetActionMap(actionMap *inputv3.ActionMap) {
	i.actionMap = actionMap
}

func (i *inputManager) RegisterAction(actionID inputv3.ActionID, action inputv3.Action) {
	i.actionMap.AddAction(actionID, action)
}

func (i *inputManager) Update() {
	i.inputBuffer.Update()

	if i.mouseEnabled {
		x, y := ebiten.CursorPosition()
		i.cursorPosition = math2D.NewVector2D(float64(x), float64(y))
	}
}

func (i *inputManager) IsActionActive(actionID inputv3.ActionID) bool {
	action, isFound := i.actionMap.GetAction(actionID)
	if !isFound {
		return false
	}
	return i.inputBuffer.IsActionActive(action)
}
